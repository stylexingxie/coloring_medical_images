...
testB=tf.data.TFRecordDataset('./testB.tfrec')
...
def preprocess_input_train(image):
    ...
    return image
def preprocess_target_train(image):
    image=tf.image.decode_jpeg(image,channels=3)
    image=random_jitter(image)
    image=normalize(image)
    return image
...
def preprocess_input_test(image):
    ...
    image=normalize(image)
    return image
def preprocess_target_test(image):
    image=tf.image.decode_jpeg(image,channels=3)
    image=tf.image.resize(image,[256,256])
    image=normalize(image)
    return image
...
trainTargetData=trainB.cache().map(preprocess_target_train,num_parallel_calls=AUTOTUNE).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)

testInputData=testA.cache().map(preprocess_input_test,num_parallel_calls=AUTOTUNE).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
testTargetData=testB.cache().map(preprocess_target_test,num_parallel_calls=AUTOTUNE).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
...
def generate_images(model, test_input,count):
    ...
    for i in range(2):
        ...
        plt.axis('off')
    # plt.show()
    plt.savefig('./predicted/fig_'+str(count)+'.jpg')
    plt.close()
...
def train_step(real_x, real_y,step):
    ...
    with summary_writer.as_default():
        tf.summary.scalar('total_cycle_loss',total_cycle_loss,step=step)
...
sample_Data = next(iter(trainInputData))
for epoch in range(EPOCHS):
    start = time.time()

    n = 0
    for image_x, image_y in tf.data.Dataset.zip((trainInputData, trainTargetData)):
    ...
    generate_images(generator_g, sample_Data)
...
generator_g.save_weights('./generatior_weight/')
